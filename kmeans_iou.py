...
def overlap(x1, len1, x2, len2):
    ...
    # if left > right return 0 # It has been done in function box_intersection
...
def init_centroids(boxes,n_anchors):
    ...
    for centroid_index in range(0,n_anchors-1):
        ...
        distance_list = []
        cur_sum = 0
        # traverse all dataset and calc distance
        for box in boxes:
            min_distance = 1
...
def do_kmeans(n_anchors, boxes, centroids):
    ...
    for box in boxes:
        ...
        groups[group_index].append(box) # choice min dist and put box into current[k] cluster 
        loss += min_distance
    ...
    for label_file in label_files:
        ...
        for line in f:
            ...
            if len(temp) > 1:
                ...
                boxes.append(Box(float(temp[1]), float(temp[2]), float(temp[3]), float(temp[4])))
...
label_path = "/home/wk/wk_ws/yolo_ws/darknet/scripts/L01/labels/test_anchor.txt"
n_anchors = 5
